...
# Set your command prefix here. By default, it's "tnm!".
pre = 'tnm!'
...
# Before launching the bot, take note of any moderators that have access to all rooms, and copy their IDs into this list - separate each entry with commas. 
opIDs = [
...
# This is your server's color set.
colorset = {
...
# Set some default color values here. Here are the ones in the TNM Discord.
    hex(0xff0000): 'red',
    hex(0xe67e22): 'orange',
    hex(0xffff00): 'yellow',
    hex(0x00ff00): 'green',
...
# Now, the token specified in the .env is used to get access to Discord's APIs.
load_dotenv()
...
# Here's what each color means in termcolor.
# --
# Information
...
# Now, I'll set up a couple variables here.
firstConnect = True
...
async def on_ready():
    ...
    if firstConnect:
        firstConnect = False
        Information('I\'ve connected to the server.\n')
    else:
        Information('I got disconnected, but I\'m back online.\n')
...
async def on_member_join(ctx):
    ...
    try:
        ...
        dmErrorFirst = 'Hi there, ' + str(ctx.mention) + '! To get the most out of the server, you\'ll need to turn on DMs from server members - at least, from *this* server. Otherwise, you won\'t be able to host rooms or see important information. You won\'t get spammed, promise.'
            ...
            while len(unused) > 0:
                try:
                    ...
                    if fofs == 'no':
                        ...
                        fofs = fofs + 1
                    unused = [a for a in sv.roles if not a.members and a.name.startswith(str(clr) + '-')]
            Success(str(delrole) + ' unused role(s) were deleted with ' + str(fofs) + ' 404 issue(s)!\n')
        else:
            print(f'\n')
